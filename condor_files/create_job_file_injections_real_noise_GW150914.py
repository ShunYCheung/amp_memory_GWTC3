...
def make_dag_files(sub_root, dag_filename):
    ...
    for i, amp in enumerate(amplitude):
        for j, label in enumerate(events):
            job_name = len(events)*i+j+1
            dag_str += (
                f"JOB {job_name} {sub_root}{label}_amp{amp}.sub\n"
            )
    dag_str += ("PARENT 0 CHILD")
    for j in range(1, len(events)*len(amplitude)+1):
        dag_str += f" {j}"
...
###################################
command = "python /home/shunyin.cheung/amp_memory_GWTC3/reweight_injection_GW150914.py"
root_dir = "/home/shunyin.cheung/amp_memory_GWTC3"
sub_root = root_dir + '/condor_files/sub_files/injections_GW150914/'
sh_root = root_dir + '/condor_files/sh_files/injections_GW150914/'
condor_root = root_dir + '/output_condor/injections_GW150914/'
...
events = ['run1', 'run2', 'run4', 'run7', 'run13', 'run14', 'run19']
...
dag_filename = root_dir + "/condor_files/reweight_injections_GW150914_real_noise.submit"
