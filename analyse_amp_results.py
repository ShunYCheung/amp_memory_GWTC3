...
def make_results(event_name):
    ...
    try:
        ...
        print(f'Error: data file cannot be found. Skipping {event_name}')
        return None
    ...
    bf_int = sp.interpolate.interp1d(s_labels, s_bf_list)

    new_amp = np.linspace(np.min(s_labels), np.max(s_labels), 1000)
    ...
    plt.legend()
    plt.tight_layout()
    plt.savefig(f'results/{event_name}/{event_name}_amplitude_vs_posterior.pdf')
    plt.savefig(f'results/{event_name}/{event_name}_amplitude_vs_posterior.png')
    ...
    np.savetxt(f"results/{event_name}/{event_name}_amplitude_posterior_results.csv", result, delimiter=',')
    try:
...
def combine_posteriors(event_list):
